...
const MatchList = () => {
    ...
    const fetchMatchesAndClubData = async () => {
      try {
        ...
        if (user?.fanData?.followedClubs?.length > 0) {
          q = query(matchesRef, where('clubId', 'in', user.fanData.followedClubs));
        } else {
          q = query(matchesRef);
        }
  ...
  const handleGrantCommit = async matchId => {
    ...
    try {
      ...
      setMatches(prev =>
        prev.map(match => (match.id === matchId ? { ...match, ...updates } : match)),
      );
                ...
                <div>
                  ...
                  {user && (
                    <div className="text-lg text-green-600 mb-4">
                      Your Committed Grant: $
                      {((nextMatch.grants && nextMatch.grants[user.uid]) || 0).toLocaleString()}
                    </div>
                  )}
                </div>
                        ...
                        {((nextMatch.grants && nextMatch.grants[user.uid]) || 0) > 0 ? (
                          <div className="bg-blue-50 p-4 rounded-lg mb-4">
                            <p className="text-blue-800">
                              ...
                              {(
                                (nextMatch.grants && nextMatch.grants[user.uid]) ||
                                0
                              ).toLocaleString()}
                      ...
                      {grantAmount &&
                        ((nextMatch.grants && nextMatch.grants[user.uid]) || 0) > 0 && (
                          <div className="text-sm text-gray-600">
                            ...
                            {(
                              ((nextMatch.grants && nextMatch.grants[user.uid]) || 0) +
                              parseFloat(grantAmount || 0)
                            ).toLocaleString()}
                          </div>
                        )}
                    </div>
